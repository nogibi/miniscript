...
miniscript-node.js: $(HEADERS) $(SOURCES) node_bindings.cpp node-cli-pre.js
	em++ -O3 -g0 -Wall -std=c++17 -fno-rtti -flto -Ibitcoin $(SOURCES) node_bindings.cpp --pre-js node-cli-pre.js -s WASM=1 -s SINGLE_FILE=1 -s FILESYSTEM=0 -s ENVIRONMENT=node -s DISABLE_EXCEPTION_CATCHING=0 -s EXPORTED_FUNCTIONS='["_miniscript_compile","_miniscript_analyze","_malloc","_free"]' -s EXPORTED_RUNTIME_METHODS='["cwrap","UTF8ToString"]' -o miniscript.js
	printf '%s\n' '#!/usr/bin/env node' > miniscript.js.tmp
	cat miniscript.js >> miniscript.js.tmp
	mv miniscript.js.tmp miniscript.js
